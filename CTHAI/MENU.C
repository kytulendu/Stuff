...
int Menu( int x, int y, MenuItem mnu[] ) {
	...
	do {
		...
		switch ( ch ) {
		case KEY_UP:
		k_up : if ( i>0 ) { go_before_menu( mnu ); i--; } else if ( i == 0 ) {
			i = n - 1;
		...
		k_dn : go_next_menu( mnu ); i++;
			...
			if ( ( mnu[menu_pt].com == M_SEPARATOR ) || ( Is_Disable_Menu( mnu[menu_pt].com ) ) )
				goto k_dn;
			break;
		case KEY_RIGHT: if ( mnu[menu_pt].com == M_POPUP )
			Write_KB( KEY_ENTER );
						else if ( menu_slide ) {
							ch = KEY_ESC;
						}
						break;
		case KEY_LEFT: if ( menu_slide ) {
			ch = KEY_ESC;
		}
					   break;
		case KEY_ENTER: {int old_menu_pt = menu_pt, old_i = i;
			if ( mnu[menu_pt].com == M_POPUP ) {
				...
				if ( menu_slide && ( Get_LastKey( ) == KEY_RIGHT ) ) {
					...
					ret = 0;
					   ...
					   break;
		default: {   int old_menu_pt = menu_pt, old_i = i;
			ch = toupper( ch );
			if ( isalnum( ch ) ) {
				menu_pt = i = 0;
...
int Pull_Menu( int x, int y, int sp, MenuItem mnu[] ) {
	...
	do {
		...
		switch ( ch ) {
		case KEY_LEFT: if ( i>0 ) { go_before_menu( mnu ); i--; } else if ( i == 0 ) {
			i = n - 1;
			while ( mnu[menu_pt].com != M_END ) go_next_menu( mnu );
			go_before_menu( mnu );
		}
					   break;
		case KEY_RIGHT: go_next_menu( mnu ); i++;
			if ( mnu[menu_pt].com == M_END ) {
				i = 0;
				menu_pt = 0;
			}
			break;
		case KEY_DOWN: Write_KB( KEY_ENTER ); break;
		case KEY_ENTER: {int old_menu_pt = menu_pt, old_i = i;
			if ( mnu[menu_pt].com == M_POPUP ) {
				menu_slide = 1;
